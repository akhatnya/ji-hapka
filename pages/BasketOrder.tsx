...
const Basket = () => {
    ...
    const [curImage, setCurImage]: any = useState([])

    useEffect(() => {
        if (basketItems.length > 0) {
            for (let i = 0; i < basketItems.length; i++) {
                loadItemDetails(basketItems[i], (res: any) => {
                    setItems(() => [...items, res.data]);
                    setCurImage([res.data.itemPhotos[0].photo.url, ...curImage])
                })
            }
        }
                ...
                <div className="row">
                    <div className="col-md-6">
                        ...
                        <div className="basket-inner-list mb-24">
                            {items.map((item: any, i: any): any => {
                                return <BasketList key={item.id} inBlock={false} inRight={true} className="add-btn mb-24" backgroundImage={`url(${curImage[i]})`} title={item.item.nameRu} price={item.item.price} />
                            })}
                            {/* <BasketList inBlock={false} inRight={true} className="add-btn mb-24" backgroundImage={'url(../images/products/product-2.png'} title="Трио стол журнальный item.nameRu" price="99 000 item.price" />
